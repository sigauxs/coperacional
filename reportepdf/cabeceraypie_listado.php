...
function Header()
{
	...
	while($row = $resultado2 -> fetch_assoc()){
		...
		$this -> Image('./imgCabecera/Celda.png',$this -> GetX(),$this -> GetY(), 60, $AltoCelda2);
		$this -> Cell(60, $AltoCelda2, utf8_decode($row['LOCACION']), 0, 0, 'C', 0);
		$this -> Cell(4, $AltoCelda2, '', 0, 0, 'L', 0);
		$this -> Image('./imgCabecera/Celda.png',$this -> GetX(),$this -> GetY(), 60, $AltoCelda2);
		...
		if ($cadena <= 110){
			$this -> Cell(188, $AltoCelda2*2, utf8_decode($row['ACTIVIDAD']), 0, 1, 'C', 0);
			$this -> Ln(6);
		}else{
			$this -> MultiCell(188, 4, utf8_decode($row['ACTIVIDAD']), 0, 'C', 0);
			$this -> Ln(6);
		}
